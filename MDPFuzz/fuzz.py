...
class fuzzing:
    ...
    def mutation(self, states):
        ...
        delta_states = np.matrix(
                [[np.random.uniform(s_min[i, 0], s_max[i, 0])] for i in range(len(s_min))],
            )
        # if np.sum(delta_states) == 0:
        #     delta_states[0] = 1
        mutate_states = states + delta_states
        mutate_states = np.remainder(mutate_states, 4)
        mutate_states = np.clip(mutate_states, -0.5, 0.1)
...
if __name__ == "__main__":
    ...
    # print(DDPG_args)
    DDPG_args["enable_test"] = True
    DDPG_args["test_episodes"] = 1
    fuzzer = fuzzing()
    # for i in range(50):
    env = ENV_CLASSES["cartpole"]()
    with open("configs.json") as f:
